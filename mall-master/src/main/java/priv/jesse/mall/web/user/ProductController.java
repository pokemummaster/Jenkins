...
public class ProductController {
    ...
    public ResultBean<Product> getProduct(int id,HttpServletRequest request) {
    ...
    public ResultBean<List<Product>> getHotProductbylevel(String level) {
        List<Product> products = productService.findByLevel(level);
        return new ResultBean<>(products);
    }
    ...
    public ResultBean<List<Product>> getNewProductone(String name) {
        if(name=="")
        {
            List<Product> list3=null;
            return new ResultBean<>(list3);
        }
       List<Product> list2= productService.findByTitleLike("%"+name+"%");
    ...
    @RequestMapping("/category2.html")
    public String toCatePage2(String level,Map<String, Object> map) {
        map.put("level", level);

        return "mall/category2";
    }
