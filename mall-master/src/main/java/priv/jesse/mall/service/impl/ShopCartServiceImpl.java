...
public class ShopCartServiceImpl implements ShopCartService {
    ...
    public List<OrderItem> listCart(HttpServletRequest request) throws Exception {
        ...
        for (Integer productId : productIds) {
                ...
                orderItem.setSubTotal((orderItem.getSubTotal()/(count-1))*(count));
                productMap.put(productId, orderItem);//覆盖
            }
        }
        List<OrderItem> orderItems = new ArrayList<>(productMap.values());//对于ID 订单信息
        return orderItems;
    }
}
